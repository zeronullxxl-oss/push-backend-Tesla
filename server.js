...
app.get('/api/analytics', (req, res) => {
  ...
  // Funnel — get test lead_ids to exclude their events
  const testLeadIds = db.prepare("SELECT lead_id FROM leads WHERE status='test'").all().map(r=>r.lead_id);
  
  // Funnel steps from events, but exclude test leads for lead-related steps
  ...
  const nonTestLeads = db.prepare("SELECT COUNT(*) as c FROM leads WHERE status!='test'").get().c;
  fb['lead_complete'] = nonTestLeads;
  fb['name_filled'] = nonTestLeads; // if they completed, they filled name
  fb['phone_filled'] = nonTestLeads; // if they completed, they filled phone
  ...
  const ls={};
  db.prepare("SELECT status,COUNT(*) as c FROM leads WHERE status!='test' GROUP BY status").all().forEach(r=>{ls[r.status||'new']=r.c;});
