...
console.log('[Config] SUPABASE_KEY:', SUPABASE_KEY ? SUPABASE_KEY.slice(0, 20) + '...' : 'NOT SET');
...
app.post('/api/subscribe', async (req, res) => {
  ...
  if (error) console.error('[Subscribe]', error.message);
...
app.post('/api/lead', async (req, res) => {
  const l = req.body;
  console.log('[Lead] Received:', JSON.stringify({ leadId: l.leadId, firstName: l.firstName, email: l.email }));
  
  const insertData = {
    lead_id: l.leadId || 'TM-' + Date.now(),
  ...
  console.log('[Lead] Inserting into Supabase...');
  const { data, error } = await supabase.from('leads').insert(insertData).select();

  if (error) {
    console.error('[Lead] SUPABASE ERROR:', JSON.stringify(error));
...
app.get('/api/test-db', async (req, res) => {
  try {
    ...
    if (error) {
      ...
      return res.json({ success: false, error: error.message, code: error.code, details: error.details, hint: error.hint });
    }
    console.log('[Test] SUCCESS:', data);
    res.json({ success: true, data });
  } catch (e) {
    console.error('[Test] CATCH:', e.message);
    res.json({ success: false, catch: e.message });
  }
});
